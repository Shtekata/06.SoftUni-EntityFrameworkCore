...
namespace MiniORM
{
    ...
    internal class ChangeTracker<T>
        where T : class, new()
    {
        private readonly List<T> allEnties;
        ...
        public ChangeTracker(IEnumerable<T> entities)
        {
            added = new List<T>();
            removed = new List<T>();
            allEnties = CloneEntities(entities);
        }

        public IReadOnlyCollection<T> AllEntityes => allEnties.AsReadOnly();
        ...
        public IEnumerable<T>GetModifiedEntities(DbSet<T> dbSet)
        {
            ...
            foreach (var proxiEntity in AllEntityes)
            {
        ...
        private static bool IsModified(T proxyEntity,T realEntity)
        {
            ...
            var modifiedProperties = monitoredProperties.Where(pi => !Equals(pi.GetValue(realEntity), pi.GetValue(proxyEntity))).ToArray();
